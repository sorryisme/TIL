...
2. Parallel Collector
   ...
   - Young 영역에서 병렬로 처리한다
   - 많은 CPU를 사용하기 때문에 GC 부하를 줄이고 어플리케이션 처리량을 증가시킬 수 있다.
   - Old영역은 Mark-sweep-compact  알고리즘 사용
3. Parallel Compacting Collector
   - Young 영역은 동일 Old 영역에서 새로운 알고리즘 사용
     - 표시단계 : 살아있는 객체를 식별하여 표시하는 단계
     - 종합단계 : 이전에 GC를 수행하여 컴팩션된 영역에 살아 있는 객체의 위치를 조사
     - 컴팩션 단계 : 컴팩션을 수행하여 컴팩션된 영역과 비어있는 영역으로 구분
   - 여러 CPU를 사용하는 서버에 적합
4. Concurrent Mark_sweep Collector
   - 힙 메모리 영역이 클 때 적합
   - Young 영역은 병렬 콜렉터와 동일
   - Old영역은 다음과 같은 단계를 거친다
     - 초기 표시 단계 : 매우 짧은 대기 시간으로 살아있는 객체를 찾는 단계
     - 컨커런트 표시 단계 : 서버 수행과 동시에 살아있는 객체에 표시를 해놓는 단계
     - 재표시 단계 : 컨커런트 표시 단계에서 표시하는 동안 변경된 객체에 대해서 다시 표시하는 단계
     - 컨커런트 스윕단계 : 표시되어있는 쓰레기를 정리하는 단계
   - CMS는 컴팩션 단계를 거치지 않다 메모리를 몰아넣지 않으므로 중간에 빈공간이 발생
   - 2개 이상 프로세서를 사용하는 서버에 적합(웹서버)
   - 점진적 방식을 지원
     - Young GC를 잘게 쪼개어 서버 대기시간을 줄일 수 있다.
     - CPU가 많지 않고 대기 시간이 짧아야할 때 사용
5. Garbage First Collector
   - Young과 Old로 나누어지지않고 바둑판 모양으로 구성(약 2000개)
   - 바둑판 모양이 각각 Eden, Suvivor, Old 영역 역할을 바꿔 가며 처리
     - 몇 개의 구역을 선정하여 Young영역으로 지정
     - 객체가 생성하면 데이터가 쌓인다
     - Young 영역으로 할당 구역 데이터가 꽉 차면 GC 수행
     - GC를 수행하면서 Survivor 구역으로 이동
   - aging을 통해서 Old영역으로 승격
   - Old 영역은 CMS GC 방식과 비슷
     - 초기 표시 (STW) : Survivor 영역의 객체를 참조하는 있는 객체를 표시
     - 기본 구역 스캔 : Old 영역 참조를 위해 Survivor 영역을 훑는다.
     - 컨커런트 표시 단계 : 전체 힙 영역에 살아있는 객체를 찾는다. Young GC가 동작하면 잠시 멈춤
     - 재 표시 (STW) : 힙에 살아 있는 객체들의 표시 작업을 완료
     - 청소: 살아있는 객체와 비어있는 객체를 식별 후 필요없는 객체 제거 , 비어있는 구역 초기화
     - 복사단계(STW) : 살아있는 객체를 비어있는 구역으로 모은다
